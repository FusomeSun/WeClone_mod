...
class DataProcessor:
    ...
    def load_csv(self, file_path) -> List[ChatMessage]:
        ...
        做整体第一次预处理，过滤不符合条件的行
        """
        df = pd.read_csv(file_path, encoding="utf-8", dtype={"content": str})
        ...
        # 如果type_name为文本 并且content包含敏感内容则删除这行
        rows_to_drop = []
        for i in df.index:
            if df.loc[i, "type_name"] == "文本":
                try:
                    ...
                    msg_str = parsed_data["msg"]
                    
                    # 保存解析出的 msg 到 msg 列
                    df.loc[i, "msg"] = msg_str
                    
                    # 检查敏感内容
                    
                    # 如果有 src 字段，也保存下来
                    if "src" in parsed_data:
                        df.loc[i, "src"] = parsed_data["src"]
        ...
        try:
            ...
            df["CreateTime"] = df["CreateTime"][1:-1]
            df["CreateTime"] = pd.to_datetime(df["CreateTime"])
        
        # 确保只选择 ChatMessage 需要的列
        # 根据错误，ChatMessage 应该需要 10 个参数
        required_columns = ["id", "MsgSvrID", "type_name", "is_sender", "talker", "room_name", "msg", "src", "CreateTime"]
        ...
        if selected_df["CreateTime"].isna().any():
            print("Warning: Still have NaT values in CreateTime, filling with current time")
            selected_df["CreateTime"] = selected_df["CreateTime"].fillna(pd.Timestamp.now())
    
        # 返回 ChatMessage 对象列表
        return [ChatMessage(*row) for row in selected_df.values]
